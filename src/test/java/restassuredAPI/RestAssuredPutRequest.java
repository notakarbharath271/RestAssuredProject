...
public class RestAssuredPutRequest {
	...
	public static void main(String[] args) {
		...
		/*String jsonresponse = "{\"username\" : \"admin\",\"password\" : \"password123\"}";

		RequestSpecification restSpec = RestAssured.given();
		restSpec.contentType(ContentType.JSON);
		restSpec.baseUri("https://restful-booker.herokuapp.com/auth");
		restSpec.body(jsonresponse);
		Response response = restSpec.post();
		System.out.println(response.toString());
		ValidatableResponse respoVali = response.then();
		respoVali.statusCode(200);
